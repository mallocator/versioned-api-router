 ...
 */
exports.configure = function(context, { path, method, api}) {
    if (!api || typeof api === 'function' || method == 'param') {
        return (req, res, next) => next();
    }
    api.paramOrder = api.paramOrder || context.globalConfiguration.paramOrder;
    api.paramMap = api.paramMap || context.globalConfiguration.paramMap || 'args';
    if (api.params) {
        for (let param in api.params) {
            var parsed = exports.parseParam(api.params[param]);
            parsed.error = parsed.error | api.error || context.globalConfiguration.error;
            parsed.validate = parsed.validate || api.validate || context.globalConfiguration.validate;
            parsed.success = parsed.success || api.success || context.globalConfiguration.success;
            api.params[param] = parsed;
        }
    }
    context.endpoints[path] = context.endpoints[path] || {};
    context.endpoints[path][method.toUpperCase()] = api;
    return exports.verify;
};
